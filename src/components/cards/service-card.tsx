...
export function ServiceCard({
  ...
  return (
    <motion.article
      ...
      className="group relative min-h-[360px] overflow-hidden rounded-[8px] border border-white/10 bg-white/[0.035] p-6"
    >
      <div className="absolute inset-0 opacity-0 transition-opacity duration-500 group-hover:opacity-100">
        <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-acid to-transparent" />
        <div className="absolute -right-24 -top-24 h-64 w-64 rounded-full bg-acid/15 blur-3xl" />
      </div>
      <div className="relative flex h-full flex-col justify-between">
        <div className="flex items-center justify-between">
          <span className="font-display text-5xl font-black text-white/12">{eyebrow}</span>
          <span className="grid h-12 w-12 place-items-center rounded-full bg-acid text-carbon">
